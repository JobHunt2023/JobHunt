...
const uploadCV =async (req: Request, res: Response): Promise<void> =>{
    try {
    ...
    //   user.cv = req.file.buffer.toString('base64');
      await user.save();
  ...
  const updateCV=async(req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const userId = req.params.userId;
      const user = await UserModel.findById(userId);
  ...
  const updateImage = async(req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const userId = req.params.userId;
      const user = await UserModel.findById(userId);
